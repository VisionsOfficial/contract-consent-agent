...
export class DataStreamProvider extends DataProvider {
  ...
  private async connectToDatabase(url: string): Promise<Db> {
    ...
    const existingConnection = DataStreamProvider.connections.get(connectionKey);
    ...
    try {
      ...
      DataStreamProvider.connections.set(connectionKey, { db, client });
      return db;
    } catch (error) {
  ...
  public static async disconnectFromDatabase(
    ...
    if (existingConnection) {
      try {
        ...
        DataStreamProvider.connections.set(connectionKey, undefined);
  ...
  async ensureReady(): Promise<void> {
    ...
    this.collection = DataStreamProvider.createCollectionProxy(
