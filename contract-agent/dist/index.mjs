  ...
  findOneAndPush(criteria, data) {
    return __async(this, null, function* () {
      ...
      Object.keys(data).map((key) => {
        data[key].map((element) => element._id = new ObjectId());
...
var _ChangeStreamDataProvider = class _ChangeStreamDataProvider extends DataProvider {
  ...
  connectToDatabase(url) {
    return __async(this, null, function* () {
      ...
      try {
        const client = yield MongoClient2.connect(url);
        const db = client.db(this.dbName);
        Logger.info("MongoDB connected successfully");
        this.db = db;
        this.client = client;
  ...
  delete(id) {
    return __async(this, null, function* () {
      try {
        const result = yield this.collection.deleteOne({ _id: new ObjectId2(id) });
        if (result.deletedCount === 0) {
          Logger.warn(`No document found with id: ${id}`);
          return false;
        }
  ...
  findOneAndPush(criteria, data) {
    return __async(this, null, function* () {
      const query = this.makeQuery(criteria.conditions);
      Object.keys(data).map((key) => {
        data[key].map((element) => element._id = new ObjectId2());
        data[key] = { $each: data[key] };
      });
      return yield this.collection.findOneAndUpdate(query, { $push: data }, { returnDocument: "after" });
    });
    ...
    return __async(this, null, function* () {
      ...
      for (const profile of allProfiles) {
        ...
        if (recommendations.dataExchanges.includes(fullDocument._id)) {
          const pull = { "recommendations.dataExchanges": { "_id": new ObjectId3(fullDocument._id) } };
          const criteria = {
            conditions: [
              {
                field: "uri",
    ...
    return __async(this, null, function* () {
      ...
      for (const profile of allProfiles) {
        ...
        if (recommendations.consents.includes(fullDocument._id)) {
          const pull = { "recommendations.consents": { "_id": new ObjectId3(fullDocument._id) } };
          const criteria = {
            conditions: [
              {
                field: "uri",
...
var _ConsentAgentRequestHandler = class _ConsentAgentRequestHandler {
  ...
  deletePreferenceByIdFromProfile(profileURL, preferenceId) {
    return __async(this, null, function* () {
      ...
      const data = { preference: { _id: new ObjectId4(preferenceId) } };
...
export {
  ...
  MongoDBProvider,
  MongooseProvider,
  agent_contract_negotiation_router_default as NegotiationAgentRouter,
  Profile
};
